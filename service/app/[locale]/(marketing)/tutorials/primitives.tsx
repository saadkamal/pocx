...
/**
 * Shared styling primitives for the tutorials page. Content lives in
...
export function GuideHeading({
  ...
  return (
    <h2
      id={id}
      className="mt-16 flex flex-wrap items-center justify-between gap-3 scroll-mt-24 border-b border-ink-200 pb-3 text-2xl font-semibold tracking-tight text-ink-900 first:mt-0"
    >
      <a href={`#${id}`} className="hover:text-brand">
        {children}
      </a>
      <span className="rounded-full border border-ink-200 px-2.5 py-1 font-mono text-xs font-medium tracking-wide text-ink-500">
        {time}
      </span>
    </h2>
  );
...
export function UiPath({ children }: { children: ReactNode }) {
  return (
    <span className="rounded bg-ink-100 px-1.5 py-0.5 text-[0.9em] font-semibold text-ink-700">
      {children}
    </span>
  );
}
...
export function Steps({ children }: { children: ReactNode }) {
  return (
    <ol className="mt-6 list-none space-y-6 [counter-reset:step]">
      {children}
    </ol>
  );
}
